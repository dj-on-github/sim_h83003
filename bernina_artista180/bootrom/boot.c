...
/* H'000FB6. Offers the link to a host for 500 rounds, polling both serial
 * channels in turn. A host claims it by sending "EB".
 *
 * The two channels are not treated alike. On SCI0 the characters are matched
 * silently. On SCI1 each matched character is echoed, and anything else draws
 * a 'Q' and restarts the match -- which is why a machine sitting in this loop
 * answers 'Q' to every stray byte.
 *
 * The channel that completes the match is left selected in CHAN_SELECTION, so
 * this is also what decides which port the download protocol then runs on. A
 * timeout leaves SCI1 selected and returns 0.
 */
static int host_handshake(void)
{
    u8 match0 = 0;   /* R4H: characters matched so far on SCI0 */
    u8 match1;       /* R4L: the same for SCI1 */
    u32 round;
    ...
    for (round = 0; round != 500; round++) {
        ...
        CHAN_SELECTION |= CHAN_SCI0;                       /* SCI0 */
        if (get_sci_rx_ready_bit()) {
            if (read_serial_data_byte() == HANDSHAKE[match0]) {
                if (++match0 == 2) return 1;
            } else {
                match0 = 0;
            }
        } else {
        ...
        CHAN_SELECTION &= (u8)~CHAN_SCI0;                  /* SCI1 */
        if (get_sci_rx_ready_bit()) {
            if (read_serial_data_byte() == HANDSHAKE[match1]) {
                send_serial_data_byte(HANDSHAKE[match1]);
                if (++match1 == 2) return 1;
            } else {
...
/* H'002341 in the original: the machine announcing itself. */
#define BOOT_STRING         "BOS"

/* The byte at H'002340 in the original, sitting immediately before the "BOS"
 * boot string. H'0C is 12, which agrees with the "BiosVersion: 1.20" line in
 * the identify banner. */
#define BIOS_VERSION        0x0C
...
static void state_identify(void)
{
    ...
    puts_serial("BiosVersion: 1.20\r");
    puts_serial("July 97\r");
    BOOT_STATE_1E = ST_IDLE;
}
