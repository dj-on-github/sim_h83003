...
/* The banner the 'I' command reports. Overridable so that an image built to
 * be installed by an update differs from the one already on the machine in
 * its body and not only in its header -- otherwise a half-written image
 * still checksums correctly, because the two bodies are the same bytes. */
#ifndef BIOS_BANNER
#define BIOS_BANNER         "BiosVersion: 1.10\r"
#endif
#ifndef BIOS_DATE
#define BIOS_DATE           "Mai 97\r"
#endif
...
/* The reset entry.
 *
 * In the stock ROM this is a link-time symbol, because the vector table and
 * the boot sequence are built together. Under v5 they are not: an image has
 * no symbol for stage-0, which lives in the permanent block. Stage-0 leaves
 * its own address here on the way past, so 'X' re-runs the image selection
 * rather than re-entering whichever image is already up.
 *
 * Reading slot 0 of the vector table would work on the hardware and does not
 * work in C: that is a load from address 0, which the compiler may treat as
 * unreachable and delete -- which it did, leaving 'X' to fall out of the
 * handler and the state byte stuck at H'3C. */
...
static void state_identify(void)
{
    ...
    puts_serial(BIOS_BANNER);
    puts_serial(BIOS_DATE);
    BOOT_STATE_1E = ST_IDLE;
}
